...
interface RawMemory {
    ...
    /**
     * An object with asynchronous memory segments available on this tick. Each object key is the segment ID
     * with data in string values. Use RawMemory.setActiveSegments to fetch segments on the next tick.
     * Segments data is saved automatically in the end of the tick.
     */
    segments: any;

    /**
     * Request memory segments using the list of their IDs. Memory segments will
     * become available on the next tick in RawMemory.segments object.
     */
...
declare class RoomVisual {
    ...
    /**
     * Draw a polygon.
     * @param points An array of roompositions, i.e. [[0,0], [5,5], [5,10]].
     * @param style The (optional) style.
     * @returns The RoomVisual object, for chaining.
     */
     ...
     * @returns The RoomVisual object, for chaining.
     */
    text(text: string, x: number, y: number, font?: TextStyle): RoomVisual;
    /**
...
declare class Spawn extends OwnedStructure {
     ...
     * @param dryrun If dryRun is true, the operation will only check if it is possible to create a creep.
     */
    spawnCreep(body: string[], name: string, opts?: {memory?: any, energyStructures?: Structure[], dryrun?: boolean}): number
